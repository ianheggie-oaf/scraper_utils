...
require_relative "fiber_operation"
  ...
  module Scheduler
    ...
    class FiberRegistry
      ...
      def register(fiber, authority)
        ...
        operation = FiberOperation.new(fiber, authority)
      ...
      def current_authority
        lookup(Fiber.current.object_id)&.authority
      end
      ...
      # Cleanup dead
      def remove_dead_operations
        operations.values.each do |operation|
          deregister(operation.authority) unless operation.alive?
        end
      end
      ...
      # Find an operation that is ready to run with the earliest resume_at
      #
      # @return [Array{FiberOperation}] Operations ready to run in resume_at order
      def ready_to_run
        @operations
          .values
          .select { |op| op.ready_to_run? }
          .sort_by(&:resume_at)
      end

      # Save the thread response into the thread and mark that it can continue
      def process_thread_response(response)
        operation = lookup(response.authority)
      ...
      # # @param fiber.object_id [Integer] The fiber's object_id
      # # @return [FiberOperation, nil] The state for the fiber or nil if not found
      # def self.state_for(fiber.object_id)
      #   fiber_states[fiber.object_id]
      # end
      #
      # # Get current fiber's state
      # # @return [FiberOperation, nil] The state for the current fiber or nil if not in a registered fiber
      ...
      # #
      # # @return [ThreadPool] the thread scheduler instance
      # def self.thread_scheduler
      #   @thread_scheduler ||= ThreadPool.new
      # end
      #
      ...
      #
      #   fiber = Fiber.current
      #   fiber.object_id = fiber.object_id
      #   state = state_for(fiber.object_id)
      #
      #   # Mark fiber as waiting for response
      #   state.waiting_for_response = true
      #
      #   # Create and queue the request
      #   request = NetworkRequest.new(fiber.object_id, client, method, args)
      ...
      #   fiber.object_id = current_fiber.object_id
      #   state = state_for(fiber.object_id)
      ...
      #   fiber_states[fiber.object_id] = FiberOperation.new(fiber.object_id, authority)
      ...
      #     state = state_for(response.fiber.object_id)
      #     next unless state
      #
      ...
      #
      #     if DebugUtils.basic?
      #       log "Received response for fiber #{response.fiber.object_id} in #{response.time_taken.round(3)}s"
      #     end
      #   end
      #
      #   true
      ...
      #     fiber.object_id = ready_fiber.object_id
      #     state = state_for(fiber.object_id)
      #
      #     if state.response_ready?
      #       log "Resuming fiber #{fiber.object_id} with response ready"
      #     elsif state.resume_at
      #       tardiness = now - state.resume_at
      #       log "Resuming fiber #{fiber.object_id} #{tardiness > 0 ? "#{tardiness.round(3)}s late" : "on time"}"
      #     end
      #   end
      #
      #   ready_fiber
