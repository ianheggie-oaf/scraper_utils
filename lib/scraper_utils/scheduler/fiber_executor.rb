...
module ScraperUtils
  module Scheduler
    ...
    module FiberExecutor
      ...
      #     ZZZRegistry.update_metrics(:delay_requested, seconds)
      #
      #     current_fiber = Fiber.current
      #
      #     # Fall back to plain sleep if scheduling not possible
      #     if !ZZZRegistry.enabled? || !current_fiber || ZZZRegistry.registry.size <= 1
      #       ZZZRegistry.update_metrics(:time_slept, seconds)
      #       ScraperUtils::LogUtils.log("Sleeping #{seconds.round(3)} seconds") if DebugUtils.basic?
      ...
      #     state = ZZZRegistry.state_for(fiber_id)
      ...
      #       ZZZRegistry.update_metrics(:time_slept, remaining)
      #       ScraperUtils::LogUtils.log("Sleeping remaining #{remaining.round(3)} seconds") if DebugUtils.basic?
      #       sleep(remaining)
      #     end || 0
      #   end
      #
      ...
      #   def run_operations
      #     count = ZZZRegistry.registry.size
      #
      #     # Main scheduling loop
      #     while ZZZRegistry.registry.any?
      ...
      #           state = ZZZRegistry.state_for(fiber_id)
      #
      #           ZZZRegistry.update_metrics(:resume_count, 1)
      #
      ...
      #           fiber.resume
      #         else
      #           ScraperUtils::LogUtils.log "WARNING: fiber is dead but did not remove itself from registry! #{fiber.object_id}"
      #           ZZZRegistry.registry.delete(fiber)
      #           ZZZRegistry.fiber_states.delete(fiber.object_id)
      #         end
      #       else
      ...
      #
      #     # Log results
      #     metrics = ZZZRegistry.delay_metrics
      #
      ...
      #
      #     puts
      #     ScraperUtils::LogUtils.log "FiberScheduler processed #{metrics[:resume_count]} calls to delay for #{count} registrations, " \
      #         "sleeping #{percent_slept}% (#{metrics[:slept].round(1)}) of the " \
      #         "#{metrics[:requested].round(1)} seconds requested."
      #     puts
      #
      #     ZZZRegistry.exceptions
      #   end
      #
      ...
      #       state = ZZZRegistry.state_for(response.external_id)
      #       next unless state
      #
      ...
      #
      #       if DebugUtils.basic?
      #         ScraperUtils::LogUtils.log "Received response for fiber #{response.external_id} in #{response.time_taken.round(3)}s"
      #       end
      #     end
      #
      #     true
      ...
      #     ZZZRegistry.registry.each do |fiber|
      #       fiber_id = fiber.object_id
      #       state = ZZZRegistry.state_for(fiber_id)
      #       next unless state
      #
      ...
      #
      #       if state.response_ready?
      #         ScraperUtils::LogUtils.log "Resuming fiber #{fiber_id} with response ready"
      #       elsif state.resume_at
      #         tardiness = now - state.resume_at
      #         ScraperUtils::LogUtils.log "Resuming fiber #{fiber_id} #{tardiness > 0 ? "#{tardiness.round(3)}s late" : "on time"}"
      #       end
      #     end
      #
      #     ready_fiber
